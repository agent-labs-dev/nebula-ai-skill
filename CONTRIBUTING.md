...
| `nebula-ai/scripts/nebula.sh` | POSIX wrapper that fixes flags, output format, and exit codes. |
...
| `tests/test_wrapper.sh` | Wrapper tests against a stub CLI. |
...
shellcheck nebula-ai/scripts/nebula.sh tests/*.sh tools/*.sh
```

`check_skill.py` verifies that every `nebula-ai` command in the docs and every
command the wrapper runs exists, with valid options, in the snapshot. It also
...
3. Update `SKILL.md` frontmatter (`compatibility` and `metadata.cli-version`),
   the references, the wrapper, and the README compatibility table.
...
- Keep the wrapper POSIX `sh` and free of dependencies beyond the CLI.
